...
class Fraction(object):
    ...
    def reduce(self):
        """ Returns a new fraction after simplfying the function """
        num_fac = [x for x in range(1,self.num+1) if self.num % x == 0]
        denom_fac = [x for x in range(1,self.num+1) if self.denom % x == 0]
        cd = [x for x in num_fac if x in denom_fac]
        gcd = max(cd)
...
class intSet(object):
    ...
    def insert(self, e):
        ...
        if not e in self.vals:
            assert type(e) == int, "ints not useds"
            self.vals.append(e)
